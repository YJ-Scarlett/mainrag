...
async function request(path, options={}) { let role='';try{role=JSON.parse(localStorage.getItem('mainrag-user'))?.role||''}catch{}const headers={...(options.headers||{}),...(role?{'X-Role':role}:{})};const r=await fetch(API+path,{...options,headers}); const data=await r.json(); if(!r.ok) throw new Error(data.detail||'请求失败'); return data; }
...
function Knowledge({user}) {
  ...
  const [docs,setDocs]=useState([]),[selected,setSelected]=useState(null),[uploading,setUploading]=useState(false),[progress,setProgress]=useState(0),[stage,setStage]=useState(''),[uploadName,setUploadName]=useState(''),[msg,setMsg]=useState('');
  ...
  return <><section className="knowledge-head"><div><span className="eyebrow"><Database size={15}/>{isTeacher?'知识中枢':'课程资源'}</span><h1>{isTeacher?'课程知识库':'我的课程资料'}</h1><p>{isTeacher?'上传、查看和管理课程资料，文件会自动建立检索索引。':'查看教师上传的课程文档，选择资料即可阅读解析后的正文。'}</p></div>{isTeacher&&<label className="primary upload-btn"><Upload/>{uploading?'正在处理…':'上传资料'}<input type="file" accept=".doc,.docx,.ppt,.pptx,.pdf" onChange={upload} disabled={uploading}/></label>}</section>{uploading&&<div className="upload-progress"><div className="progress-file"><i><FileText/></i><span><b>{uploadName}</b><small>{stage}</small></span><strong>{progress}%</strong></div><div className="progress-track"><i style={{width:progress+'%'}}/></div></div>}{msg&&<div className="notice">{msg}</div>}<div className="kb-summary"><div><Database/><span><b>{docs.length}</b><small>知识文档</small></span></div><div><FileText/><span><b>{docs.reduce((n,d)=>n+d.chunks,0)}</b><small>知识片段</small></span></div><div><Sparkles/><span><b>可阅读</b><small>资料状态</small></span></div></div><section className="panel table-panel"><div className="panel-head"><div><h3>{isTeacher?'全部资料':'教师共享资料'}</h3><p>支持 DOC、DOCX、PPT、PPTX、PDF</p></div></div><div className="doc-table"><div className="tr th"><span>资料名称</span><span>分类</span><span>片段</span><span>上传时间</span><span>操作</span></div>{docs.map(d=><div className="tr" key={d.id}><span className="doc-name"><i><FileText/></i><b>{d.name}<small>{d.size} KB</small></b></span><span><em className="tag">{d.type}</em></span><span>{d.chunks}</span><span>{d.created_at}</span><span className="doc-actions"><button className="view-doc" onClick={()=>view(d.id)}><BookOpen/>查看</button>{isTeacher&&<button className="trash" onClick={()=>del(d.id)}><Trash2/></button>}</span></div>)}</div></section>{selected&&<section className="panel document-reader"><div className="reader-head"><div><span className="eyebrow"><FileText size={14}/>{selected.type}</span><h2>{selected.name}</h2><p>{selected.created_at} · {selected.size} KB</p></div><button onClick={()=>setSelected(null)}><X/></button></div>{selected.has_preview?<iframe className="document-frame" src={`${API}/knowledge/${selected.id}/preview`} title={selected.name}/>:<div className="no-preview"><AlertCircle/><b>暂无原版式预览</b><p>这是内置演示资料，请由教师重新上传原始文件后查看。</p><pre>{selected.content}</pre></div>}</section>}</>
}

function Analysis({role}){const [data,setData]=useState(null);useEffect(()=>{request(role==='teacher'?'/analysis/class':'/analysis/student').then(setData)},[role]);const mastery=data?.mastery||[];return <><section className="analysis-title"><div><span className="eyebrow"><ChartNoAxesCombined size={15}/>{role==='teacher'?'班级数据洞察':'个性化学习报告'}</span><h1>{role==='teacher'?'班级学情分析':'我的学情分析'}</h1><p>{role==='teacher'?'基于学习记录与问答行为，快速掌握班级学习状态。':'看见自己的进步，也找到下一步努力的方向。'}</p></div></section><div className="stats"><Stat icon={ChartNoAxesCombined} label="平均掌握度" value={(data?.summary.average||0)+'%'} detail="综合学习活动" tone="blue"/><Stat icon={BookOpen} label="学习活动" value={data?.summary.activities||0} detail="已纳入分析" tone="green"/><Stat icon={MessageCircle} label="主动提问" value={data?.summary.questions||0} detail="探索意识良好" tone="violet"/><Stat icon={Database} label="课程资料" value={data?.summary.documents||0} detail="可供检索" tone="orange"/></div><div className="analysis-grid"><section className="panel"><div className="panel-head"><div><h3>知识点掌握度</h3><p>按学习活动综合评估</p></div></div><div className="chart"><ResponsiveContainer><BarChart data={mastery} layout="vertical" margin={{left:20,right:30}}><CartesianGrid stroke="#eef1f7" horizontal={false}/><XAxis type="number" domain={[0,100]} axisLine={false}/><YAxis type="category" dataKey="topic" width={90} axisLine={false} tickLine={false}/><Tooltip/><Bar dataKey="score" fill="#627eea" radius={[0,8,8,0]} barSize={20}/></BarChart></ResponsiveContainer></div></section><section className="panel insight"><div className="insight-icon"><Sparkles/></div><h3>智能学习建议</h3><p>{data?.suggestion}</p><div className="insight-list">{mastery.slice(0,3).map((m,i)=><div key={m.topic}><span>{i+1}</span><b>{m.topic}<small>{m.score<70?'建议重点复习':m.score<85?'继续巩固':'掌握良好'}</small></b><em>{m.score}%</em></div>)}</div></section></div>{role==='teacher'&&<section className="panel student-panel"><div className="panel-head"><div><h3>学生概览</h3><p>班级个体学习状态</p></div></div><div className="student-list">{data?.students?.map((s,i)=><div key={s.name}><span className="student-avatar">{s.name[0]}</span><b>{s.name}<small>{s.activities} 次学习活动</small></b><div className="mini-progress"><i style={{width:s.average+'%'}}/></div><em>{s.average}%</em></div>)}</div></section>}</>}
